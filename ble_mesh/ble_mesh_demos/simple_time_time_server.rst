...
_`示例功能简介`
==================
本示例功能主要实现SIG 标准的 time time server model，可以用于灯等设备模型。
本示例实例化了两个element，每个element包括一个 time server model，初始化部分
可以参考examples 目录下 mesh_app.c 文件里面的 mesh_app_init_user函数说明，开发者
可以非常容易添加更多的model。每个model初始化需要开发者初始化相关的控制接口，在例子
程序中user_time_0_evt_cb，user_time_1_evt_cb分别作为两个开发者接口，开发者完成自己
...
* 节点上有两个time time server，可以通过手机单独控制任一 server。
...
* 指示灯
  ...
  * led1 :
       ...
       * 蓝灯
                * 常亮:
                    * light lightness server **1** 设置亮度的 lightness 为 0x8000；
       * 红灯
                * 熄灭， 保留；
                * 常亮， 保留；
  * led2 :
       ...
       * 红灯
                * 常亮:
                    * light lightness server **2** 设置亮度的 lightness 为 0x8000；

软件环境
********************************
* 设备端运行 ble mesh sdk 的 examples 目录下 simple_time_time_server示例。
...
设备正常上电后：
  * led1 :
       ...
       * 蓝灯
                * 常亮， time time server **1** 默认设置打开亮度 为 50%；
       * 红灯
                * 熄灭， 保留；
  * led2 :
       * 绿灯
       ...
       * 蓝灯
                * 熄灭， relay 功能默认关闭；
       * 红灯
                * 常亮， time time server **2** 默认设置打开亮度 为 50%；
