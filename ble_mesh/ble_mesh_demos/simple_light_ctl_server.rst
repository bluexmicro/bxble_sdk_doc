...
_`示例功能简介`
==================
本示例功能主要实现SIG 标准的 light ctl server model，可以用于灯等设备模型。
本示例实例化了两个element，每个element包括一个 ctl server model，初始化部分
可以参考examples 目录下 mesh_app.c 文件里面的 mesh_app_init_user函数说明，开发者
可以非常容易添加更多的model。每个model初始化需要开发者初始化相关的控制接口，在例子
程序中user_ctl_0_evt_cb，user_ctl_1_evt_cb分别作为两个开发者接口，通过控制灯
...
* 指示灯
  ...
  * led1 :
       ...
       * 蓝灯
                * 熄灭:
                    * light ctl server **1** 设置亮度的 lightness 值设置为 0；
                * 不同亮度:
                    * light ctl server **1** 设置亮度的 lightness 为 0 的不同亮度,
                        并且还与 temperature有关；
       * 红灯
                * 熄灭， 保留；
                * 常亮， 保留；
...
软件环境
********************************
* 设备端运行 ble mesh sdk 的 examples 目录下 simple_light_ctl_server示例。
...
设备正常上电后：
  * led1 :
       ...
       * 蓝灯
                * 常亮， light ctl server **1** 默认设置打开亮度 lightness 为 50%,此时 lightness 的值为0x8000；
       * 红灯
                * 常亮， 亮度为默认 lightness 和 temerature 值综合的结果；
  * led2 :
       * 绿灯
                * 闪烁， 设备正常工作；
       * 蓝灯
                * 熄灭， relay 功能默认关闭；
       * 红灯
                * 常亮， light ctl server **2** 默认设置打开亮度 ctl 为 50%,此时 lightness 的值为0x8000；
